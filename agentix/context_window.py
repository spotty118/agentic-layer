...
def estimate_tokens(text: str) -> int:
    ...
    Uses a more sophisticated approach than simple character division:
    - Counts words and punctuation separately
    - Accounts for common tokenization patterns
    - More accurate than len(text)//4 but doesn't require external libraries
    ...
    # - Long words (>8 chars) often split into multiple tokens
    token_count = 0
    for word in words:
        if word.isalnum():
            # Words: estimate 1 token per 4 chars, minimum 1
            token_count += max(1, len(word) // 4)
        else:
            # Punctuation and special chars: 1 token each
            token_count += 1

    # Add a small overhead for formatting/special tokens
...
class SharedContextWindow:
    ...
    def get_messages(
        ...
        include_provider_attribution: bool = True
    ) -> List[Dict[str, Any]]:
        """
        ...
        Args:
            ...
            include_provider_attribution: Add provider/model info to assistant messages
        ...
        if format == "openai":
            result = []
            for m in messages:
                msg = m.to_openai_format()
                # Add provider attribution for assistant messages if enabled
                if include_provider_attribution and m.role == "assistant" and m.provider:
                    attribution = f"[Generated by {m.provider}"
                    if m.model:
                        attribution += f" using {m.model}"
                    attribution += "]\n\n"
                    msg["content"] = attribution + msg["content"]
                result.append(msg)
            return result
        elif format == "full":
    ...
    def _trim_if_needed(self):
        ...
        while self.total_tokens > self.max_tokens and len(self.messages) > 1:
            ...
            # Update token count using improved estimation
            self.total_tokens -= estimate_tokens(removed.content)
